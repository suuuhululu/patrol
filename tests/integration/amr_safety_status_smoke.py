...
ROBOT_ID = "robot1"
SOURCE_SESSION_ID = "robot1-20260908T210000"
NS = f"/{ROBOT_ID}"
...
def _status_stamps(node, since_index):
    ...
    return [
        status.header.stamp.sec + status.header.stamp.nanosec / 1e9
        for status in node.status_observations[since_index:]
    ]


def _check_publication_rate(node, launch_process, log_path):
    """Q-02: regular 2 Hz, and change publication capped at 10 Hz.

    PublicationGate implements both numbers, but the smoke only ever
    counted messages -- it never measured the interval between them, so a
    ...
    while time.monotonic() < quiet_deadline:
        rclpy.spin_once(node, timeout_sec=0.02)
    stamps = _status_stamps(node, start)
    if len(stamps) < 4:
        raise AssertionError(
            f"expected at least 4 regular publications, got {len(stamps)}"
        )
    intervals = [b - a for a, b in zip(stamps, stamps[1:])]
    # TICK_SECONDS 가 0.02초라 0.5초 경계는 한 tick 늦게 잡힐 수 있다.
    if not all(0.45 <= gap <= 0.60 for gap in intervals):
        raise AssertionError(
            f"regular publication is not 2 Hz: {[round(g, 3) for g in intervals]!r}"
        )

    # 2) 변경 폭주. enum 축을 50 Hz 로 흔들어도 10 Hz 를 넘으면 안 된다.
    start = len(node.status_observations)
    storm_deadline = time.monotonic() + 1.5
    toggle = 0
    while time.monotonic() < storm_deadline:
        toggle ^= 1
        node.battery_status_publisher.publish(
            UInt8(data=RobotStatus.NORMAL if toggle else RobotStatus.LOW)
        )
        rclpy.spin_once(node, timeout_sec=0.02)
    stamps = _status_stamps(node, start)
    if len(stamps) < 5:
        raise AssertionError(
            f"expected change publications during the storm, got {len(stamps)}"
        )
    intervals = [b - a for a, b in zip(stamps, stamps[1:])]
    fastest = min(intervals)
    if fastest < 0.09:
        raise AssertionError(
            "change publication exceeded the 10 Hz cap: fastest interval "
            f"{fastest:.3f}s, all={[round(g, 3) for g in intervals]!r}"
        )
    if fastest > 0.20:
        raise AssertionError(
            "a changing enum did not raise the rate above the 2 Hz floor: "
            f"fastest interval {fastest:.3f}s"
        )
    return len(stamps), fastest
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            print(
                ...
                f"(storm={change_count} msgs, fastest={fastest:.3f}s)"
            )
            print(
