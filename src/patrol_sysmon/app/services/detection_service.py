"""ReportDetection 서비스 요청의 검증과 저장.

요청 6개 필드(로봇 ID · 사건 ID · 시각 · 위치 · 사진 · 종류)를 검증해 사진 파일 한 장과
events 한 행으로 저장한다. 중복 판정은 event_id + 내용 해시로 한다.
"""
...
# [제품 범위] 화재·누수·장애물 세 종류만 저장한다. 서비스 요청의 event_type 1·2·3에 대응한다.
EVENT_TYPES = {"FIRE", "LEAK", "OBSTACLE"}


class DetectionValidationError(ValueError):
    """ReportDetection 요청 값이 규격과 다를 때 사용한다."""
...
def validate_report(payload, now=None):
    """ReportDetection 요청 6개 필드를 검증해 저장용 record와 이미지를 돌려준다."""
...
def receive_report(payload, now=None):
    ...
    반환 outcome: "accepted"(저장) 또는 "duplicate"(같은 event_id·같은 내용, 또는 억제).
    값 오류는 DetectionValidationError, 같은 event_id에 다른 내용은 DetectionMessageConflictError.
    """
    ...
    if not image_path.exists():
        temporary_path = None
        try:
            # [원자적 파일 저장] 완성되지 않은 사진이 조회되지 않도록 이름을 마지막에 바꾼다.
    ...
    try:
        ...
        # [DB 실패 정리] 이번 요청이 새로 만든 파일만 제거하고 기존 사진은 보존한다.
        if created:
