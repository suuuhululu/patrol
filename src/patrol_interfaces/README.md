...
- DriveToken은 `control_session_id`·`token_id`·`message_sequence`, RobotStatus는 `*_state`와 구조화 ID 필드, EStop은 `target_robot_id`·`reason`·`latched`를 사용한다. PatrolReport의 `reason_code`는 문서와 같은 uint32다.
- CommandCheck의 check_state 숫자, RobotStatus safety enum 숫자와 waypoint·scan 동작, EStop reason 숫자·전체 대상 값은 각각 TBD-IF-001·003·004로 남는다. 미정 enum 상수는 `.msg`에 임의로 추가하지 않았다.
