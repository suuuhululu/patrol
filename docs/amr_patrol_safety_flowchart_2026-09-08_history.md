> **과거 구현·시험 이력 보관본 — 최신 설계·실기 계획이 아님.** 2026-09-09 개편 전 본문을 보존한다. 과거 시점의 미정·완료 표현은 [현재 작업 문서](amr_patrol_safety_flowchart.md)를 우선한다.

# `amr_patrol_safety` 문서 기준 노드·모듈·메시지 연결도

작성일: 2026-09-08 · 상태: **담당 코드 연결·로컬 ROS 검증 완료(임시 정책 포함), 실기 대기** · 담당 범위: AMR 로컬 안전·배터리·상태/결과·명령 식별 입구

2026-09-08 사용자 재확인 기준 작업 범위는 `patrol_amr_safety`다. 실제 패키지 경로는 `src/patrol_amr_safety/`이며 이 문서의 파일명에 남아 있는 `amr_patrol_safety`는 이전 표기다. 패키지 이름은 변경하지 않는다.
...
> 최신 완료 범위는 바로 아래 체크포인트를 따른다. 담당 코드 연결과 로컬 ROS 검증은 임시 정책을 포함해 완료했으며, 실제 TurtleBot 4 시험·팀 간 통합·임시 정책의 확정 계약 교체는 남아 있다. 코드별 그림의 대조 여부와 과거 시험 이력은 각각의 절에 표시한다.

### 최신 코드 완료 기준 — 2026-09-08 23:29 KST

사용자의 후속 지시인 **미정 보고 규칙을 임시로 정하고 별도 명시하여 코드를 먼저 완성**하는 기준으로, `patrol_amr_safety` 네 노드의 작성·연결 및 로컬 ROS 검증을 마쳤다. 이는 전체 AMR 시스템, 실제 mission/Nav2 실행, TurtleBot 4 실기 또는 공용 TBD 확정 완료가 아니다. 임시 상태 규칙의 기준 문서는 [임시 결정·합병 교체 지점](development/provisional-amr-status-policy.md)이며, 이전 입력 계약 대기 기록보다 이 후속 지시를 우선한다.

| 담당 기능·완료 요건 | 현재 코드와 근거 | 판정 |
|---|---|---|
| 배터리 분류·유지·미수신 | battery_monitor, 단위·배터리 ROS 및 전체 launch 결과(7.0절) | 로컬 구현·검증 완료 |
| token·heartbeat·E-stop·최종 속도 | SafetyGate·공통 guard, 두 namespace 안전 시험과 heartbeat 단독 단절 시험 | 로컬 구현·검증 완료 |
| 실제 정지 상태 | supervisor odom 구독·공통 정지 기준·stale 후 STOPPING | 로컬 구현·검증 완료, 실물 odom 미확인 |
| 명령 접수·중복·상태·영속 DB | command_gateway, 두 namespace 명령 6종·충돌·재시작 시험 | 로컬 구현·검증 완료, 실제 mission 실행 별도 |
| RobotStatus 기본 입력·주기 | status_reporter, 상태·ID·pose·odom·2 Hz/최대 10 Hz ROS 시험 | 로컬 구현·검증 완료 |
| 운영·도킹·scan 입력 누락 대응 | provisional_status_policy→_tick→RobotStatus, 임시 정책 단위·production ROS 시험 | **사용자 승인 임시 규칙으로 코드 연결 완료** |
| PatrolReport 변환·보존·복구 | adapter, 성공/실패/취소·미구독·재시작·변환 오류·복구 ROS 시험 | v1.0 로컬 구현·검증 완료, 외부 ACK는 차기 계약 |
| 두 로봇 분리·launch | 같은 domain의 두 production launch, 토픽·상태·명령·SQLite 분리 | 로컬 동시 시험 통과 |
| 코드별 흐름·임시 결정·인수인계 | 3절 flowchart, 별도 임시 정책 문서, 7.0절 미실행 목록 | 문서 반영 완료 |

마지막 코드 변경(임시 상태 연결)의 safety 빌드는 `1 package finished [2.87s]`, 전체 단위시험은 **367개 통과**(14.262초)다. `/tmp/patrol_provisional_units.log`에 기록했다. 관련 ROS 회귀는 **AMR07_STATUS_REPORTER_SMOKE_PASS** 및 **AMR_SMOKE_PASS**이며, `/tmp/provisional_amr07_status_reporter_smoke.log`와 `/tmp/provisional_amr_safety_status_smoke.log`에 있다. 전자는 임시 세 상태의 실제 전달까지, 후자는 기존 안전 경로와 발행 주기 유지를 확인한다. 이전 기능의 검증 범위·로그는 7.0절에서 구분한다.

**남은 작업은 실기·통합과 임시 규칙의 합병 교체다.** TurtleBot 4 관련 미실행 시험은 7.0절의 2026-09-09 오전 사용자 확인 목록을 유지한다. 외부 송신·commit·push·배포는 수행하지 않았다.
...
flowchart LR
    ...
    DRIVER -->|odom<br/>Odometry<br/>안전 정지 완료 판단| LOCAL
...
flowchart TD
    ...
    ODOM[odom callback] --> MEASURE[RobotStatusState.observe_odometry]
    MEASURE --> SAFETY
    AGE --> SAFETY
...
제어 입력 오류 경로도 ROS 콜백 안에서 처리한다. `_on_drive_token`, `_on_estop`, `_on_heartbeat`는 기존 guard가 거절한 `ValueError`를 기록하고 노드를 유지한다. 무효 메시지로 sequence·lease를 갱신하지 않으며, 기존 유효 상태와 기존 timeout에 따라 출력을 다시 계산한다. unknown enum을 유효 값으로 바꾸거나 새 복구 조건을 만들지 않는다. 대상은 `5f185b4` 기반 현재 작업 트리이며 다음 추가 흐름은 코드와 대조했다.

```mermaid
flowchart TD
    INPUT[_on_drive_token·_on_estop·_on_heartbeat] --> VALIDATE[기존 guard의 입력 검증]
    VALIDATE -->|유효| OBSERVE[기존 observe·순서 판정 반영]
    VALIDATE -->|ValueError| WARN[잘못된 입력 경고·노드 유지]
    OBSERVE --> STATE[_publish_if_changed]
    WARN --> STATE
    STATE --> OUT[_publish_output<br/>기존 권한·후보 신선도로 차단 판단]
    OUT --> TIMER[기존 0.1초 timer 유지]
```
...
**2026-09-08 사용자 결정:** `local_safety_supervisor`가 odom을 직접 받아 `STOPPING → STOPPED`를 판단한다. 실제 정지 수치는 [interfaces.md 3·4절](interfaces.md)을 따르며 `robot_status_state.py`의 기존 공통 구현을 재사용한다. status_reporter는 수신 safety_state를 그대로 보고한다. 판단 주체 미정은 해결됐으며 [수신 단위 검토 요청 초안](change_requests/CR-AMR_09-08_23-02_실제_정지_safety_state_판정.md)에 영향과 변경 불필요 근거를 기록했다.

아래는 `src/patrol_amr_safety/patrol_amr_safety/local_safety_supervisor.py`의 추가 경로다. 상태: **구현 대조 완료**, 대상 `5f185b4` 기반 2026-09-08 작업 트리. 시험 결과는 7.0절에 별도로 기록한다.

```mermaid
flowchart TD
    RX[LocalSafetySupervisor._on_odometry] --> OBS[SafetyGate.observe_odometry]
    OBS --> SHARED[robot_status_state.py<br/>RobotStatusState.observe_odometry]
    SHARED -->|무효·역순 ValueError| WARN[경고·이전 유효 측정은 신선도까지만 유지]
    SHARED -->|정상 측정| RECHECK[_publish_if_changed]
    WARN --> RECHECK
    TIMER[_recheck 0.1초 timer] --> RECHECK
    INPUT[token·heartbeat·E-stop·정상 후보 callback] --> RECHECK
    RECHECK --> STATE[SafetyGate.safety_state]
    STATE --> ES{E-stop 활성?}
    ES -->|예| ESTOP[ESTOPPED]
    ES -->|아니오| BLOCK{SafetyGate.output 차단 사유 있음?}
    BLOCK -->|아니오| NORMAL[NORMAL]
    BLOCK -->|예| STOP{공통 snapshot.motion_stopped?}
    STOP -->|예| STOPPED[STOPPED]
    STOP -->|아니오·미수신·stale·재이동| STOPPING[STOPPING]
    ESTOP --> PUB[변경 시 safety_state UInt8 발행]
    NORMAL --> PUB
    STOPPED --> PUB
    STOPPING --> PUB
```

공통 측정 흐름은 [상태 모델의 odometry 구현 설명](amr.md)과 `robot_status_state.py`를 참조한다. 후보 timeout에 따른 출력 차단도 이 상태에 반영하지만, 후보 유무는 기존 `motion_allowed` 권한 판정을 바꾸지 않는다. `amr_safety_status.launch.py`의 `odom_topic`은 두 노드에 동일하게 remap한다.
...
| `provisional_status_policy.py` | 사용자 승인 임시 운영·도킹·scan 추정. 합병 시 단일 교체 모듈 |
...
flowchart TD
    ...
    BRIDGE --> MODEL
    MODEL --> PROJECT[provisional_status_policy.project_axes<br/>임시 운영·도킹·scan 보고 규칙]
    PROJECT --> GATE{최초·정기 0.5초<br/>또는 상태 변경 0.1초?}
...
2026-09-08 후속 사용자 지시로 운영·도킹·scan 입력 미합의는 코드 연결의 중단 조건에서 제외했다. **임시 보고 규칙**을 [별도 결정 문서](development/provisional-amr-status-policy.md)에 모았으며 공용 계약 확정으로 간주하지 않는다. 기존 생산자 JSON은 그대로 읽고 안전 패키지 안에서 보고 필드를 채운다. 임시 추정이 실제 주행 허가·도킹 Action·Detection을 실행하지 않는다.

아래 신규 모듈·호출 경로는 **구현 대조 완료**, `5f185b4` 기반 현재 작업 트리다.

```mermaid
flowchart TD
    TICK[StatusReporter._tick] --> INPUT[RobotStatusState.snapshot<br/>MissionStatusBridge.snapshot·has_snapshot]
    INPUT --> POLICY[provisional_status_policy.project_axes]
    POLICY --> OP[_operational<br/>임시 우선순위로 enum 선택]
    POLICY --> DOCK[_docking<br/>mission·직전 docking·battery]
    POLICY --> SCAN[_scan<br/>mission·safety·실측 정지]
    OP --> AXES[StatusAxes 반환]
    DOCK --> AXES
    SCAN --> AXES
    AXES --> UPDATE[update_states·update_mission_context]
    UPDATE --> CHANGED{운영·도킹 enum 변경?}
    CHANGED -->|예| NOTE[PublicationGate.note_change]
    CHANGED -->|아니오| DUE[PublicationGate.due]
    NOTE --> DUE
    DUE -->|예| PUB[_publish<br/>임시 3개 필드 포함 RobotStatus]
    DUE -->|아니오| WAIT[다음 tick]
```

실제 임시값·우선순위·한계·합병 교체 지점은 위 별도 문서만 기준으로 관리한다. 임시 정책의 시작 로그 ID는 `TEMP-AMR-STATUS-20260908-v1`이다.

`src/patrol_amr_safety/patrol_amr_safety/patrol_report_adapter.py`의 실패 경로를 보강했다. ROS Time 범위를 넘는 결과 시간이나 메시지 변환 오류도 `PatrolReportPublishError`로 전달하므로 reporter의 오류 기록 경로에서 처리하고 pending 원본을 보존한다. 상태: **구현 대조 완료**, 대상 `5f185b4` 기반 2026-09-08 작업 트리.

```mermaid
flowchart TD
    POLL[StatusReporter._poll_mission_and_reports] --> DRAIN[PatrolReportDrain.publish_pending]
    DRAIN --> SUB{report 구독자 연결?}
    SUB -->|아니오| KEEP[pending 유지·다음 poll 대기]
    SUB -->|예| READ[outbox.pending]
    READ -->|파일·schema 오류| FILEERR[PatrolReportOutboxError]
    READ --> RECORD[미전송 record 순회]
    RECORD --> FILL[message_factory·fill_message]
    FILL --> TIME[nanoseconds_to_time<br/>정수·비음수·ROS Time 범위 검사]
    TIME -->|유효| PUB[publisher.publish]
    PUB --> REMOVE[outbox.mark_published]
    REMOVE -->|성공| RECORD
    FILL -->|변환 오류| ERR[PatrolReportPublishError]
    TIME -->|무효| ERR
    PUB -->|발행 오류| ERR
    REMOVE -->|저장 오류·record 사라짐| ERR
    FILEERR --> LOG[reporter 오류 기록·노드 유지]
    ERR --> LOG
    LOG --> KEEP
```

삭제는 기존 v1.0 임시 기준인 구독자 연결·publish 호출 성공을 유지한다. 외부 DB 저장 ACK가 확인됐다는 뜻이 아니며, TBD-IF-003의 삭제 계약을 이번 오류 처리로 확정하지 않는다.
...
2026-09-08 gateway 시작 경로 수정: 파일명만 지정한 상대 DB 경로에서도 부모 디렉터리를 절대 경로로 계산해 생성한다. 저장소 경로 자체는 사용자 지정값을 유지한다. `tests/integration/command_lifecycle_smoke.py`는 임시 작업 디렉터리와 상대 DB 파일명으로 production 노드를 기동해 검증한다.

아래 시작 경로는 **구현 대조 완료**, 대상 `5f185b4` 기반 2026-09-08 작업 트리다.

```mermaid
flowchart TD
    MAIN[command_gateway.main] --> INIT[CommandGateway.__init__]
    INIT --> CONFIG[validate_configuration]
    CONFIG -->|잘못된 robot·session| FAIL[ValueError·시작 실패]
    CONFIG -->|유효| PATH[지정 database_path 또는 default_database_path]
    PATH --> PARENT[abspath의 부모 디렉터리 생성]
    PARENT -->|파일 권한·경로 오류| FAILPATH[시작 실패·오류 노출]
    PARENT --> DB[CommandStore 열기]
    DB --> PRUNE[_prune Q-14]
    PRUNE --> RX[구독·타이머로 명령과 lifecycle 처리]
    RX --> END[destroy_node에서 store.close]
```
...
| `SAFETY_STOPPING → STOPPED` | **2026-09-08 사용자 결정: local_safety_supervisor 직접 odom 판정** | 3.2절 구현·수신 단위 검토 요청 참조, 실기 미실행 |
...
| 안전·상태 launch | `amr_safety_status.launch.py`가 battery, local safety, status reporter, command gateway 네 노드를 기동하도록 정리했다. 시험용 토픽과 DB/JSON 경로를 모두 별도 namespace·임시 경로로 바꿀 수 있다. | **구현 완료, 2026-09-08 격리 수정 후 전체 ROS smoke 통과** |
...
| 7.2 안전 | guard·motion 단위시험과 전체 launch smoke 일부 | token·heartbeat·E-stop 조합, 후보 속도 timeout, odom·pose·battery 전달 | **단위시험·전체 launch ROS smoke 통과. 정기 2 Hz, 변경 폭주 구간 최단 간격 0.100초 확인** |
...
| 전체 단위 회귀 | Python unittest 전체 | 마지막 launch 격리 인자 수정분을 포함한 현재 코드 회귀 | **2026-09-08 재실행: 355개 통과, `OK` (15.296초)** |
| ROS 빌드 | `patrol_interfaces`, `patrol_amr`, `patrol_amr_safety` colcon build | 공용 메시지 생성과 두 패키지 import·launch 설치 | **2026-09-08 재빌드 통과: `3 packages finished [43.6s]`** |
| TurtleBot 4 실기 | 실행하지 않음 | 실제 driver topic·QoS·센서값·구동 반응 | **미실행** |

이전 중단 지점은 7.2의 `amr_safety_status_smoke.py` 전체 launch 시험이었다. 처음에는 시험 domain에서 예상하지 못한 odometry가 들어와 실제 환경과 섞일 가능성을 발견했다. 그 실행은 종료했고, 이후 모든 시험 입력과 최종 `cmd_vel` 출력을 `/patrol_test` 아래로 remap하고 DB·JSON도 임시 디렉터리로 분리했다. 격리 전 실행에서 후보 속도 `0.25 m/s`가 `/robot1/cmd_vel`에 잠깐 발행됐을 가능성이 있으므로, 이력상 안전 주의사항으로 남긴다. 당시에는 격리 수정 후 문법 검사까지만 수행했다. 아래 재개 검증에서 전체 ROS smoke 재실행을 통과했다.

#### 2026-09-08 재개 검증 기록

대상 코드 버전은 `5f185b4`이며 이번 실행에서 동작 코드는 수정하지 않았다. ROS 2 Jazzy를 source하고 아래 순서표 1번 빌드 후 `install/setup.bash`를 source하여 `python3 -m unittest discover -s tests -p 'test_*.py' -v`를 실행했다. 단위시험 로그는 `/tmp/patrol_safety_unit_20260908.log`, 빌드 로그는 저장소 `log/latest_build/`에 있다. 임시 로그는 영구 보존 자료가 아니다.

2026-09-08 사용자 정정: 작업 중 중간 보고를 제공하되 로컬 자동 단위시험·빌드·격리 ROS smoke는 별도 확인 없이 진행한다. 사용자가 직접 ROS 통신을 확인하거나 실물 로봇·다른 PC와 연결해야 하는 시험에서만 중단하여 확인받는다. 빌드·단위시험과 아래 3·4번 로컬 ROS smoke 5종은 모두 통과했다. 실물 시험과 다른 PC 연결 시험은 미실행이다. 다음 단계는 사용자 1의 실제 로봇 토픽·타입·QoS 읽기 전용 확인이다. TBD-IF-003 등 기존 미정 계약은 이번 검증으로 해결되지 않는다.

이번 ROS 자동시험은 샌드박스의 UDP 소켓 차단으로 첫 실행이 실패하여 실행 권한 승인 후 다시 수행했다. 시험 프로세스에서만 외부 Discovery Server·Fast DDS profile 환경을 제외하고 localhost discovery를 사용했다. 기존 Onboard/Offboard 설정 파일은 변경하지 않았다. 전체 launch의 센서·제어 입력과 최종 속도는 `/patrol_test` 하위, DB·JSON은 임시 경로로 격리했다. 결과는 실물 주행·관제 PC 통합 검증을 대신하지 않는다.

| 재실행 시험 | 결과 | 임시 로그 |
|---|---|---|
| 전체 launch, domain 127 | `AMR_SMOKE_PASS`, 최종 속도 발행자 `local_safety_supervisor` 1개, 상태 sequence 1~63, 변경 구간 13건·최단 0.100초 | `/tmp/patrol_safety_ros_smoke.log` |
| 배터리 ROS | `BATTERY_MONITOR_ROS_SMOKE_PASS` | `/tmp/battery_monitor_ros_smoke.log` |
| 상태·결과 ROS | `AMR07_STATUS_REPORTER_SMOKE_PASS` | `/tmp/amr07_status_reporter_smoke.log` |
| 명령 lifecycle ROS | `COMMAND_LIFECYCLE_SMOKE_PASS` | `/tmp/command_lifecycle_smoke.log` |
| gateway 재시작 ROS, domain 125 | `GATEWAY_PERSISTENCE_PASS`, 재응답·중복 dispatch 없음, prune 실행·최근 1건 보존 | `/tmp/command_gateway_persistence_smoke.log` |

마지막 시험의 최근 1건 보존 결과만으로 24시간/1,000개 전체 경계를 실물에서 검증했다고 해석하지 않는다. 이번 작업은 기존 코드 자동검증과 문서 갱신이며 동작 코드·공용 계약·타 담당 코드는 변경하지 않았다.
...
#### 2026-09-08 23:06 후속 구현·검증 체크포인트

목표는 `patrol_amr_safety` 코드 완성과 ROS 통신 검증이다. 대상 코드는 `5f185b4` 기반 현재 작업 트리이며 commit·push는 수행하지 않았다.

| 변경·검증 | 결과 | 근거 |
|---|---|---|
| 실제 정지 상태 | 사용자 결정대로 local safety odom 구독·공통 측정 판정·launch remap 구현 | 3.2절 flowchart, [검토 요청 초안](change_requests/CR-AMR_09-08_23-02_실제_정지_safety_state_판정.md) |
| gateway 상대 DB 경로 | 파일명만 주어도 기동되도록 수정 | 3.4절 시작 flowchart |
| safety 패키지 빌드 | 통과, `1 package finished [2.76s]` | `log/latest_build/` |
| 전체 단위시험 | **360개 통과**, 14.268초 | `/tmp/patrol_safety_implementation_units.log` |
| robot1·robot6 격리 safety ROS | 각각 **AMR_SMOKE_PASS**. odom 없이 STOPPED 없음, 연속 정지 후 STOPPED, stale 후 STOPPING, 최종 속도 발행자 1개, 2 Hz/최대 10 Hz 확인 | `/tmp/patrol_safety_robot1_measured_stop.log`, `/tmp/patrol_safety_robot6_measured_stop.log` |
| robot1·robot6 gateway ROS | 각각 **COMMAND_LIFECYCLE_SMOKE_PASS**. 상대 DB, 6종 접수·dispatch, 잘못된 robot/enum/target 거절, 중복 무dispatch, payload 충돌 203, lifecycle·report replay | `/tmp/patrol_gateway_robot1_matrix.log`, `/tmp/patrol_gateway_robot6_matrix.log` |

두 namespace 시험은 순차 로컬 실행이다. 두 실물 로봇 동시 시험이나 mission/Nav2 실행 완료를 의미하지 않는다. gateway의 6종 시험은 입력 검증과 내부 전달까지이며 각 시나리오의 실행 가능 여부·STOP/CANCEL 최종 결과를 검증하지 않는다.

**남은 코드·계약 경계:** operational_state·docking_state·scan_state는 현재 mission snapshot에 입력이 없어 전체 상태 연결 완료가 아니다. [입력 연결 검토 초안](change_requests/CR-AMR_09-08_23-06_운영_도킹_상태_입력_연결.md)에 생산자·수신자 작업을 분리했다. 공용 application ACK는 v1.0 밖 TBD-IF-003, STOP 결과·재시작 세부는 기존 결정 범위와 TBD-AMR-005를 따르며 임의로 구현하지 않는다. 외부 수신 단위 검토 요청서는 파일로 작성했으며 메시지로 전달하지 않았다.

#### 2026-09-08 23:14 보고 실패·복구 검증

odom 안전 상태·gateway 수정 검증에 이어 상태·결과 보고의 실패·복구 경로를 실제 ROS로 확인했다.

- `patrol_report_adapter.py`: ROS Time에 표현할 수 없는 시간 및 메시지 변환 오류를 report 발행 오류로 감싸 pending을 보존하고 노드를 유지한다. 코드별 flowchart는 3.3절에 있다.
- 전체 단위시험 **362개 통과**(14.010초), safety 패키지 빌드 **통과**(3.07초). 로그 `/tmp/patrol_safety_recovery_units.log`.
- `amr07_status_reporter_smoke.py` **PASS**: 구독자 없는 상태의 보존, production reporter 재시작과 같은 report ID·원래 report 세션 유지, 유효 범위를 넘는 시간 보존·RobotStatus 계속 발행·수정 후 재전송, 손상·역순 mission snapshot의 마지막 정상 상태 유지 및 새 revision 복구, FAILED·SUCCEEDED·CANCELED 필드 전달. 로그 `/tmp/patrol_status_recovery.log`.
- `amr_safety_status_smoke.py` robot1·robot6 각각 **PASS**: token과 후보 속도를 신선하게 유지한 채 heartbeat만 중단했을 때 motion_allowed=false와 최종 0 출력을 확인했다. 이전 시험은 heartbeat를 계속 보냈으므로 이번 검증과 구분한다. 로그 `/tmp/patrol_safety_robot1_heartbeat.log`, `/tmp/patrol_safety_robot6_heartbeat.log`.

위 시험은 로컬 DDS 통신이며 외부 관제·DB 저장 ACK·실물 로봇의 결과가 아니다. 추가 통신 검증에서는 두 로봇 동시 namespace 분리 등 아직 증거가 없는 항목을 따로 확인한다. 운영·도킹·scan 입력 계약과 사용자 실기 대기는 앞선 체크포인트를 유지한다.

#### 두 namespace 동시 검증과 남은 구현 경계

- `tests/integration/amr_dual_namespace_smoke.py`는 robot1·robot6의 production safety launch를 같은 localhost domain 129에서 동시에 실행했다. 상태·mission ID·배터리·명령 dispatch·SQLite 경로 분리, 로봇별 시험 속도 토픽의 단일 발행자, token holder별 출력, 개별 대상·all E-stop을 확인해 **AMR_DUAL_NAMESPACE_SMOKE_PASS**다. 로그 `/tmp/patrol_dual_namespace.log`.
- 최종 속도와 센서 입력은 `/patrol_dual` 하위에 격리했고 실제 `/robotN/cmd_vel` 발행자가 없음을 확인했다. Nav2·mission worker·TurtleBot 4는 실행하지 않았다. 시험용 토픽의 0 출력 확인은 실물 odom 정지 확인을 대신하지 않는다.
- 남은 운영·도킹·scan 상태는 현재 production mission 코드에서 생산하지 않는다. 입력·전이 합의나 추가 자료를 사용자에게 요청했다. [입력 연결 요청서](change_requests/CR-AMR_09-08_23-06_운영_도킹_상태_입력_연결.md)의 승인 범위를 유지한다. 임의 enum 추정이나 상대 코드 변경으로 완료를 선언하지 않는다.
- 제어 입력 검증 예외가 안전 노드를 종료하지 않도록 위 3.2절 콜백 처리를 보강했다. 마지막 수정분은 관련 단위시험 37개와 robot1 기존 ROS smoke가 통과했다. 무효 token·E-stop·heartbeat가 입력돼도 노드 유지·초기 정지·후속 정상 입력 처리를 확인했다. 로그 `/tmp/patrol_control_validation_units.log`, `/tmp/patrol_control_validation_ros.log`. 이 결과를 robot6 마지막 수정분 재시험이나 실기 PASS로 확대하지 않는다.

#### 과거 중단 조건 확인 — 2026-09-08 23:22 KST (후속 임시 구현 승인으로 해제)

**후속 결정:** 사용자가 임의 규칙을 별도 명시하고 코드를 먼저 완성하도록 승인했다. 따라서 아래 중단 기록은 과거 이력이며, 현재 구현 기준은 [임시 정책](development/provisional-amr-status-policy.md)이다.

사용자는 운영·도킹·scan 상태의 mission 측 합의 내용이나 참고 자료가 **없음**을 확인했다. 세 상태의 실제 생산·전이·전달 계약은 미완성이며, 검사만 남은 상태로 표시하지 않는다. 수신 코드가 임의로 상태를 추정해서는 안 되고 현재 작업 범위 밖의 mission 생산 코드를 임의 변경해서도 안 된다.

재개 조건은 [입력 연결 요청서](change_requests/CR-AMR_09-08_23-06_운영_도킹_상태_입력_연결.md)의 입력 주체·전이·전달 schema 합의와 해당 생산 코드 반영 범위 확인이다. 이미 수정한 코드의 필요한 자동검증은 마쳤다. 같은 검사를 반복하거나 새 검사를 늘려 입력 계약 대기를 대체하지 않는다. 목표는 전체 코드·로컬 ROS 연결 완료로 유지하며 현재 달성됐다고 선언하지 않는다.

#### 로봇 관련 시험 대기 — 2026-09-09 오전(KST) 사용자 확인 예정

2026-09-08 사용자 지시: 현재 로봇을 확인할 수 없으므로 로봇 관련 시험은 **진행 필요·미실행**으로 남긴다. 사용자는 **2026-09-09 오전(KST)**에 확인할 예정이다. 이는 확인 예정이며 실물 시험 실행 승인이나 시험 완료를 의미하지 않는다. 로봇 연결·사용자 확인이 필요한 단계는 사용자가 준비되었음을 알린 뒤 진행한다.

- [ ] 실제 로봇의 토픽 이름·메시지 타입·QoS 확인 — 아래 사용자 1부터 읽기 전용으로 시작한다.
- [ ] 실제 배터리·odometry·AMCL 입력과 RobotStatus 반영 확인 — 아래 사용자 2·3.
- [ ] 최종 속도 발행자 단일성, E-stop·token 회수·heartbeat 단절 시 정지 확인 — 아래 사용자 4, 주행 시험 전 안전 조건 확인 필요.
- [ ] mission/Nav2와 명령·상태·결과 종단 확인 — 아래 사용자 5, 공동 시험 준비 필요.
- [ ] 두 로봇의 실기·도킹·네트워크 단절 및 복구 시험 — 상세 범위와 순서는 9.8절 RT-01~08, 중단 기준은 9.9절을 따른다. 오전에 모두 완료한다고 가정하지 않는다.

로컬 자동시험 통과와 실물 시험 결과는 구분한다. 실제 확인 후 시험별 일자·robot ID·코드 버전·ROS domain·관측값·로그 경로·PASS/FAIL을 기록하고, 수행하지 못한 항목은 계속 미실행으로 유지한다.
...
| 실제 정지와 safety enum | 0 출력 직후, odometry가 정지 경계를 0.5초 충족하기 전·후 | 전에는 STOPPING, 실제 정지 확인 뒤 STOPPED라는 문서 의미 유지 | 사용자 결정 반영·노드 통합 검증 |
