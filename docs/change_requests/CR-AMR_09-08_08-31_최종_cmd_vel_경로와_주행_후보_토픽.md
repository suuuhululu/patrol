...
- 관련 TBD ID: TBD-IF-009. 이 요청은 제안이며 관제 합의 전까지 OPEN을 유지한다. TBD-AMR-001(주행 중재)·TBD-AMR-006(로컬 정지·감속·장애물)은 이 요청으로 해결하지 않는다.
...
- 결정 일자·근거: 미결정. 2026-09-08 AMR이 로컬 Jazzy 설치본을 조사해 제안을 작성했다.
- 코드 변경 승인 근거·범위: 미승인. 이 요청서만 추가하며 `local_safety_supervisor.py`·launch·Nav2 설정은 합의 전까지 바꾸지 않는다.
...
### 제안 2 — 후보는 TwistStamped, 최종 출력은 Twist

후보 입력에만 `TwistStamped`를 쓰자는 제안이다.
...
### 제안 3 — 후보 신선도 기준 신설 요청 (Q-17)

후보가 끊겼을 때 최종 출력을 정지로 내리는 기준이 필요하다. [amr.md 3절](../amr.md)이 임의 timeout 추가를 금지하므로 AMR이 값을 정하지 않고 신설을 요청한다.

- 제안값: **0.5초.** 구동부 `diffdrive_controller`의 `cmd_vel_timeout: 0.5`와 같은 값이다. 새 숫자를 만들지 않고 이미 구동부가 쓰는 값에 맞추는 것이 근거다.
- 대안 후보: Nav2 `velocity_smoother`의 `velocity_timeout: 1.0`, Q-01 lease 1.0초.
- 확정 전까지 AMR은 후보 timeout을 구현하지 않는다. 현재의 token·E-stop 게이트만 적용한다.
...
- `/robotN` namespace를 Nav2 노드에 적용하는 방식(launch `namespace` 인자 대 topic prefix remap). 박성현 launch 구조에 달려 있다.
...
|---|---|---|---|
| AMR | 합의 후 `local_safety_supervisor`에 후보 구독·최종 발행 추가, `MotionGuard.evaluate()` 연결, 단위·ROS 시험 확장 | `src/patrol_amr/patrol_amr/local_safety_supervisor.py`, `launch/amr_safety_status.launch.py` | 조정묵 |
...
1. 제안 1의 토픽 이름 3개(`cmd_vel_safe`·`cmd_vel_yaw`·`cmd_vel`)와 삽입 위치에 동의하는가. `collision_monitor`를 체인에 남기는 것에 동의하는가.
2. Nav2를 `/robot1`·`/robot6` namespace로 실행하는가, 아니면 로봇별 PC에서 namespace 없이 실행하는가. 후자라면 최종 출력 이름이 `/cmd_vel`이 되므로 interfaces.md 7절의 "전역 /cmd_vel 미사용" 문장과의 관계를 확정해야 한다.
3. `enable_stamped_cmd_vel: true` 적용에 동의하는가. 동의하지 않으면 후보 신선도를 무엇으로 판정할지 정해야 한다.
4. Q-17 후보 신선도 timeout을 0.5초로 신설하는 데 동의하는가.
5. yaw 정렬 후보를 발행하는 주체가 `mission_supervisor`가 맞는가. 맞다면 AMR-18·19 `recovery_supervisor`와의 소유 경계도 함께 확인한다.
...
|---|---|---|---|
| AMR / robot1 | 미반영 | `feat/amr-safety-status` `9eb151f`. `motion_allowed`만 발행 | 관제 회신 후 후보 구독·최종 발행 구현, IT-16 |
| AMR / robot6 | 미반영 | 위와 같음 | 위와 같음 |
| 관제 | 미반영 | | 위 5개 질의 회신, Nav2 launch·params 반영 |
...
| 일자 | 검토자·단위 | 결정·의견 | 근거 |
|---|---|---|---|
| 2026-09-08 | AMR | 제안 작성. 로컬 Jazzy 설치본의 Nav2 체인·TwistPublisher·Create 3 설정을 근거로 최소 변경안 제시 | 5·6단계에서 TBD-IF-009로 비워 둔 최종 출력이 IT-16과 launch 통합을 동시에 막고 있음 |
| | 관제 | 회신 대기 | |
