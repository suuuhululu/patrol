...
브랜치명과 병합 상태는 이 계획에 고정하지 않고 작업 시점의 Git 상태로 확인한다. 코드·문서 수정 승인만으로 commit·push·fetch·merge까지 승인된 것으로 해석하지 않는다.
...
| 20 | AMR-04·05·06·07 | token·command 저장/중복 방지·RobotStatus v1.0 25필드·PatrolReport publisher/영속 outbox를 실제 mission 입력에 연결 |
| 21 | AMR-08·09·10·16 | map/TF·Keepout·safe-zone·공통 Nav2 경로를 robot1/robot6 namespace로 연결 |
| 22 | AMR-12·13 | 배터리 회귀를 유지하며 도킹 진입·timeout·DOCKED 완료 센서와 별도 충전 감지 신호 활성 2초 연속 판정·실패 보고 연결 |
| 23 | AMR-11·20 | 관제 EStop 대상·대표 reason 반영과 heartbeat watchdog을 최종 정지 경로에 연결. 물리 E-stop·수동 reset·local latch는 제외 |
...
| AMR-11 estop_guard 로컬 반영 | 40 | 관제 EStop의 robot1·robot6·all, active·reason 0~6·sequence와 해제 후 token/command 재개 gate를 최종 속도에서 확인. 물리 버튼·local latch·수동 reset은 v1.0 제외 | 23 |
| AMR-12 배터리 상태 enum 보고 | 100 | 기존 7값·경계·3초 유지·CRITICAL 즉시 시험 회귀 유지 | 22 |
| AMR-13 도킹 실행·성공 판정 | 50 | DOCKING 진입, 60초 내 DOCKED 완료 센서와 `BatteryState` enum과 독립적인 충전 감지 신호 활성 2초 연속, timeout/실패 report, 새 mission 중복 없음 | 22 |
...
| AMR-20 heartbeat watchdog | 0 | 확정된 v1.0 계약의 1.0초 초과 안전 정지와 timer·주기·timeout을 robot1·robot6 실제 ROS 경로에서 검증 | 23 |
...
| 28 | T-03 로컬 안전·E-stop 테스트 | 30 | UI OPERATOR를 포함한 비물리 E-stop reason, token 만료, Nav2/yaw 동시 후보, 충돌 속도, 최종 속도 발행자 1개와 안전 차단 우회 없음. 물리 입력은 시험하지 않음 |
...
> 아래는 시간순 누적 이력이다. 앞쪽 항목의 “미구현”·“미정”은 뒤쪽 구현·결정으로 대체될 수 있으며, 현재 공용 계약은 [v1.0 기준선](../decisions/2026-09-08-control-interface-baseline.md)을 따른다.
...
- 빨간 기본 mask는 상시, 노란 중앙통로 mask는 관제가 `/vision/cctv/patrol_allowed=false`를 받았을 때 활성하는 정책으로 자산에 분리했다. 당시 safety 패키지 분리 전 자동시험 `Ran 250 tests`/`OK`, 두 패키지 빌드 성공이다.
- 21단계 Nav2 이중 Keepout 연결로 기존 TurtleBot4 parameter에 적용하는 overlay, robot1·robot6 namespace launch, base/center mask server와 filter info server lifecycle 구성을 추가했다. 기본 filter는 항상 ON, 중앙통로 filter만 관제 transaction 대상으로 분리했다. 당시 safety 패키지 분리 전 전체 자동시험 `Ran 255 tests`/`OK`, 두 패키지 빌드와 launch 인자 검증이 성공했다. 현재 빌드는 `patrol_amr_safety`도 함께 선택한다. 관제 적용·read-back·rollback과 양쪽 로봇 실기는 [이중 Keepout parameter 요청서](../change_requests/CR-AMR_09-08_13-02_이중_Keepout_parameter_계약.md) 합의 후 남는다.
...
- **당시 초기 상태(2026-09-08):** 종단 ROS 연결을 막던 공유 계약을 [AMR 수정 요청서](../change_requests/CR-AMR_09-08_11-48_명령_상태_보고_종단_계약.md)로 분리했다. 이후 v1.0에서 메시지 숫자·payload 등 확정된 항목은 더 이상 blocker가 아니며, 상세 실행 상태 전이와 report ACK 등 차기 버전 잔여만 해당 TBD로 관리한다.
...
- **당시 초기 상태:** 20단계 AMR-05의 첫 구현으로 `command_store.py`에 재시작 영속 중복 제거, 상태·완료 report 보존, Q-14 retention을 추가했다. 당시 없던 public CommandCheck 숫자와 mission ROS adapter는 뒤의 v1.0 확정 및 `command_gateway.py` 구현으로 해소됐으며, AMR-05의 현재 잔여 gate는 3절을 따른다.
...
- 23단계 AMR-20은 `ControlHeartbeat` wire 타입, `heartbeat_guard.py`의 session·sequence·1초 초과 timeout, `local_safety_supervisor` 최종 안전 ROS 배선까지 구현했다. 현재 잔여 gate는 robot1·robot6 실제 ROS 타이밍·timeout·최종 정지 통합시험이다.
...
- 이후 v1.0 확정으로 `check_state` 0~3과 거절 reason code 203~206이 고정됐다. `command_gateway`는 충돌 203·무효 parameter 205를 호출자나 launch parameter로 받지 않고 공용 상수를 직접 사용한다.
