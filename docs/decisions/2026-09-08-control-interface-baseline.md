...
- 상태: 현재 단계 계약 확정 · 코드 반영 및 통합 검증 미실시
- 기준 소스: `main` commit `1cf0059`
- 결정 근거: [AMR 명령·Heartbeat·E-stop·상태 계약 확정 회신](../change_requests/CR-AMR_09-08_17-00_명령_Heartbeat_E-stop_상태_계약_확정_회신.md), [AMR 이중 Keepout parameter 계약](../change_requests/CR-AMR_09-08_13-02_이중_Keepout_parameter_계약.md)
...
- `src/patrol_interfaces/msg/RobotStatus.msg`에는 같은 `SAFETY_*` 상수 블록이 두 번 선언되어 있다. enum 의미와 숫자는 이 기준선대로 한 세트만 사용하며, 관제 빌드 전에 중복 선언을 제거해야 한다. 이 문서 작업에는 `.msg` 수정과 빌드 검증을 포함하지 않는다.
