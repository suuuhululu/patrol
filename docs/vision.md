...
cam_master는 event_id를 Q-13 동안 보관해 같은 이벤트를 한 번만 처리한다. CCTV 이벤트는 RELIABLE/VOLATILE이며 과거 이벤트 재생을 위해 TRANSIENT_LOCAL을 사용하지 않는다. ID 생성·state 정수값은 문서 하단 CR 반영 내역 반영으로 TBD-IF-005 결정 내용을 따른다.
...
cam_master는 patrol_allowed가 바뀌면 즉시 발행하고, 바뀌지 않아도 5Hz로 현재 값을 반복 발행한다(문서 하단 CR 반영 내역, CR-관제_09-07_17-53_비전_CameraState와_permit_반영 4절). 통신 timeout 시 마지막 patrol_allowed 값을 유지하고 시스템 모니터가 경고·로그를 남긴다. 임의로 false로 변경하지 않는다. 단, 이 정책이 token·E-stop 등 별도 안전 게이트를 무효화하지 않는다.
...
P0 반영(문서 하단 CR 반영 내역) 관련 자동 시험은 `src/patrol_vision/test/test_p0_camerastate.py`에 있다: event_id·source_session_id 형식, restart_sequence 파일 카운터 증가·손상 폴백, 0.2초 경계 상수, cam_master의 topic-camera_id 불일치 폐기, patrol_allowed 매핑·5Hz 상수, 순서 역전 폐기를 다룬다. 이 테스트는 opencv-python·ultralytics·rclpy·patrol_interfaces가 설치된 실제 빌드 환경에서 실행을 전제로 하며, 카메라·YOLO 모델이 필요한 부분(실제 프레임 처리, 0.2초 실측 경계)은 이 자동 시험 범위 밖이고 IT-05·06·07 통합시험에서 확인한다.

## TBD

TBD-VIS-001(카메라 입력·확정 기준·장애 판단)과 TBD-VIS-002(순서 역전·재시작 정책)는 2026-09-07 확정되어 각각 2-1절·4-1절 본문에 반영됐다. 남은 TBD는 없다.

발행 주기와 경고 timeout은 TBD-IF-010, CameraState 계약은 TBD-IF-005에만 정의한다. TBD-IF-005(패키지·event_id 규칙·camera_id)와 TBD-IF-010의 permit 5Hz·5초 timeout 부분은 CR-관제_09-07_17-53_비전_CameraState와_permit_반영으로 결정됐다(interfaces.md 갱신은 별도 진행). TBD-IF-010의 나머지 부분(RobotStatus 변경 발행 rate 제한 등)은 여전히 OPEN이다.
